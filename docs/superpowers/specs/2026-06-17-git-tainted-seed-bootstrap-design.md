...
| Oid algo | The wire oids are bare hex with no algo tag; the seed **infers sha1/sha256 from hex length** and persists the algo. ⚠ **Hard prerequisite (§8):** the store's decode hard-codes SHA-1 in places and `OID.Equal` is algo-sensitive, so without an algo-aware-decode companion fix a seeded **sha256** oid mis-compares against the first live sync and false-taints the tag. |
...
- **Algo-aware oid decode (C2 — hard prerequisite).** `OID.Equal` compares algo (`model/oid.go`), but the store decode hard-codes `model.SHA1` for taint-event oids (`store/sqlite/convert.go`, mirror in mysql) and `remotes.hash_algo` is generally NULL (nothing in the live create path sets it). A seeded **sha256** oid would therefore `Equal`-mismatch the first live sync and false-taint every tag. Fix: persist the remote's hash algo (the seed sets it from inference; the create API may set it too), and make the decode read it (no SHA-1 hard-coding). This is its own small, independently-testable change and a blocker for sha256 correctness — note it is a *pre-existing latent* bug the seeder is the first path to activate at scale.
